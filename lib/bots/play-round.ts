...
export async function botPlayRound({ matchId, botUserId, roundNum }: BotPlayRoundOpts): Promise<void> {
  ...
  // ── 1. Load bot profile (need bot_skill) ────────────────────────────────
  ...
  // ── 3. Load questions (need correct answers) ─────────────────────────────
  ...
  for (const qId of questionIds) {
    ...
    const timeMs = Math.floor(4000 + Math.random() * 8000) // 4-12s

    // Speed bonus: 100 + max(0, 50*(1-(time-3000)/12000)) if correct
  ...
  if (match.current_round >= 3) {
    ...
    // Advance to next round, back to a_turn
    await admin.from('matches').update({
      status: 'a_turn',
